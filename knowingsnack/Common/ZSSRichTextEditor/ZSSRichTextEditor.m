...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *, id> *)info{
    ...
    /* wangdicen:::!!!! pay attention!!!吧这些图片储存到某个数组,然后一起上传到服务器.沙盒 */
    //存图片
    NSData *imageData = UIImageJPEGRepresentation(scaledImage, 0.5);
    
    
    _imageNum ++;
    NSString *imageName = [NSString stringWithFormat:@"userEmail-articleNum-%d.png",(int)_imageNum];
    //存入沙盒
...
//    NSString *imageBase64String = [scaledImageData base64EncodedStringWithOptions:0];
    ...
    NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:imageName,@"imageName", nil];
...
//    NSString *path = [NSHomeDirectory() stringByAppendingPathComponent:@"tmp"];
    NSURL *baseURL = [NSURL fileURLWithPath:fullPath];
    [self setBaseURL:baseURL];
    [self insertImage:imageName alt:[NSString stringWithFormat:@"Image-%d",(int)_imageNum]];
